<style>
    ...
    table.table tbody td p.redcolrer,
    .redcolrer{
        color: red !important;
    }
...
@media(max-width:996px){
    ...
    <?php $aush = Auth()->user();$old_Con=0; $tj_date = date('Y-m-d');
            $paidsus_usd = 0;?>
    @foreach($yes as $key => $row)
        ...
        <?php
            ...
            else if(isset($request["st"]) && $request["st"] == "p" && $row->completed == 0){
                $row_show = true;
            }
            else if(isset($request["st"]) && $request["st"] == "f" && ($row->completed == 1 || $row->completed == 0)){
                $row_show = true;
            }
            
            $now_cd = Carbon\Carbon::parse($row->date);
            $difference = $now_cd->diffInDays();
        ...
        ?>
        @if($row_show == true)
            <tr id="tr_{{ date('d-M', strtotime($row->date)) }}" class="{{ $keysyes }} {{ $red == true ? 'sredcol':'' }}" color="{{$row->color}}">
                <td style="{{$color}}" class="{{ $old_this == true ? "CPRV": "" }}  {{ $rosw != 0 ? "has_old" : ""}}" >
                    ...
                    <div class="itemsmobilew">
                        ...
                        <a data-url="{{ route('panel.movement.delete' , $row->movement_id ) }}" class="mx-1 delete" data-bs-toggle="tooltip"
                        ...
                        @if($goblue)
                            <div class="form-check form-switch ps-0 check_center goblue">
                                ...
                                data-url="{{ route('panel.movement.change_statusr' , $row->movement_id ) }}"
                        ...
                        @if($sho && $goblue == false)
                            <div class="form-check form-switch ps-0 check_center">
                                <input class="form-check-input ms-auto is-displayed"
                                data-url="{{ route('panel.movement.change_status' , $row->movement_id ) }}"
                                type="checkbox" id="flexSwitchCheckDefault"
                                {{ $row->completed == 1 ? 'checked' : ''}}>
                            </div>
                        @endif
                        
                    </div>
                </td>
                <td class="text-center {{ $red == true ? 'redcol':'' }} {{ $row->to_date == "" ? "not_has" : ""}}" style="{{$color}}">
                        <?php 
                                    $to_date_1 = $row->to_date;
                ...
                <td class="user" style="{{$colorv}}">
                    ...
                    <a class="text-xst font-weight-bold mb-0"  href="{{ url('/admin/entries?d_user='.@$maher_user->id)}}" style="background:{{@$maher_user->background}};color:{{@$maher_user->color}}">{{ @$maher_user->user_name }}</a>
                    @endif
                    <a class="text-xst font-weight-bold mb-0"  href="{{ url('/admin/entries?d_user='.$row->m_user->id)}}" style="background:{{@$row->m_user->background}};color:{{@$row->m_user->color}}">{{ $row->m_user->user_name }}</a>
                </td>
                @endif
                @endif
                    ...
                    <?php
                        ...
                        if(isset($request["d_user"]) && sizeof(@$row->users) == 1){
                            if($request["d_user"] == $row->users->first()->id){
                                $useme2 = '<a class="text-xsm font-weight-bold mb-0" href="/admin/entries?d_user='.$row->users->first()->id.'" style="background:'.$row->users->first()->background.';color:'.$row->users->first()->color.'">DIRECT</a>';
                            }else{
                                $useme2 = '<a class="text-xsm font-weight-bold mb-0" href="/admin/entries?d_user='.$row->users->first()->id.'" style="background:'.$row->users->first()->background.';color:'.$row->users->first()->color.'">'.$row->users->first()->full_name .'</a>';
                            }
                        }
                    ?>
                        ...
                        if(isset($request["d_user"]) && sizeof($row->users) == 1){
                            ...
                            if(isset($request["d_user"])){
                                if($request["d_user"] == $user->user->id){
                                    ...
                                    if($user->type == 0){
                                        $useme2 .= '<a class="text-xsm font-weight-bold mb-0" href="/admin/entries?d_user='.$user->user->id.'" style="background:'.@$user->user->background.';color:'.@$user->user->color.'">'.@$user->user->full_name .'</a>';
                                    }
                                    else{
                                        $useme1 .= '<a class="text-xsm font-weight-bold mb-0" href="/admin/entries?d_user='.$user->user->id.'" style="background:'.@$user->user->background.';color:'.@$user->user->color.'">'.@$user->user->full_name .'</a>';
                                    }
                                }
                            }
                            else{
                ...
                <td class="user psw" style="{{$cs_clore0}}">
                    ...
                    @else
                        @if($row->commission != 0 || $row->admin_commission != 0)
                            ...
                            <div>
                                <span>CC</span>
                                <span class="text-xst font-weight-bold mb-0">{{$row->commission_type}} {{ $row->commission }}</span>
                            </div>
                            @endif
                            @if($row->admin_commission != 0)
                            <div>
                                <span>AC</span>
                                <span class="text-xst font-weight-bold mb-0">{{$row->admin_commission_type}} {{ $row->admin_commission }}</span>
                            </div>
                            @endif
                        @endif
                        @if($row->commission == 0 &&  $row->admin_commission == 0)
                            @if($row->sec_price == null)
                                <p class="text-xst font-weight-bold mb-0 _1" {{ $row->sender_paid }}>{{ $row->price_type." ".$row->price }}
                                @if(isset($ispartner->type) && $ispartner->type == 5)
                                    @if($row->paybyus == "1" && $row->sender_paid == 0 && $row->sender_user != null)
                                    <i class="fa fa-clock-o"></i>
                                    @endif
                ...
                <td class="user psw" style="{{$cs_clore1}}">
                    ...
                    @else
                        ...
                        @if(isset($row->admin_partner) && $row->admin_partner != null)
                            ...
                            <div style="{{$cs_clore0w}}">
                                <span>AP</span>
                                <span class="text-xst font-weight-bold mb-0">{{ $row->price_type." ".$row->admin_partner }}</span>
                            </div>
                        @endif
