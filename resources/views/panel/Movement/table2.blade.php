...
@foreach ($yes as $key => $row)
    ...
    @if ($row_show == true)
        <tr id="tr_{{ date('d-M', strtotime($row->date)) }}" class="{{ $keysyes }} {{ $red == true ? 'sredcol' : '' }}"
            color="{{ $row->color }}">
            <td style="{{ $color }}"
                class="{{ $old_this == true ? 'CPRV' : '' }}  {{ $rosw != 0 ? 'has_old' : '' }}">
                    ...
                    @if ($goblue)
                        <div class="form-check form-switch ps-0 check_center goblue">
                            <input class="form-check-input ms-auto is-displayed"
                                ...
                                type="checkbox" id="flexSwitchCheckDefault" {{ $row->color == 4 ? 'checked' : '' }}>
                        </div>
                    @endif
                    @if ($sho && $goblue == false)
            ...
            <td class="text-center {{ $red == true ? 'redcol' : '' }} {{ $row->to_date == '' ? 'not_has' : '' }}"
                ...
                <p>
                    ...
                    <b class="text-xst font-weight-bold mb-0"
                        @if (in_array($row->movement_id, $troughline_ids ?? []) && $row->to_date == null) style="text-decoration: line-through" @endif>{{ date('d M', strtotime($row->date)) }}</b>

                    @if ($row->to_date != null)
                        <b @if (in_array($row->movement_id, $troughline_ids ?? [])) style="text-decoration: line-through" @endif
                ...
                @if ($aush->type != 3)
                    <td class="user" style="{{ $colorv }}">
                        @if (isset($row->middleman) && $row->middleman == '1' && $row->m_user->id != $maher_user->id)
                            <a class="text-xst font-weight-bold mb-0"
                                href="{{ url('/admin/entries?d_user=' . @$maher_user->id) }}"
                                style="background:{{ @$maher_user->background }};color:{{ @$maher_user->color }}">{{ @$maher_user->user_name }}</a>
                        @endif
                        <a class="text-xst font-weight-bold mb-0"
                            href="{{ url('/admin/entries?d_user=' . $row->m_user->id) }}"
                ...
                @else
                    ...
                    @if ($row->commission == 0 && $row->admin_commission == 0)
                        ...
                        @else
                            <div>
                                <span>P1</span>
                                <span class="text-xst font-weight-bold mb-0 _0">{{ $row->price_type }}
                                    {{ $row->price }}
                            </div>
                            <div>
                                <span>P2</span>
                    ...
                    <td class="user psw" style="{{ $cs_clore }}">
                        ...
                        @if ($row->t_net != 0)
                            <div style="{{ $cs_clore_t }}">
                                <span>TN</span>
                                <span
                                    class="text-xst font-weight-bold mb-0">{{ $row->price_type . ' ' . $row->t_net }}</span>
                            </div>
                        @endif
                    </td>
                @endif
                    ...
                    @else
                        ...
                        @if (isset($row->tax) && $row->tax != 0)
                            <div>
                                <span>TX</span>
                                <span
                                    class="text-xst font-weight-bold mb-0">{{ $row->price_type . ' ' . $row->tax }}</span>
                            </div>
                        @endif
                        @if ($row->admin_partner == null && $row->revenue_partner == null)
