    ...
    <?php
        ...
        function sumPartners($collection,$type, $currency)
        {
            ...
            if($type == "5"){
                ...
                $sum = $collection->where('price_type', $currency)->where("paybyus" , "0")->sum("revenue") +
                        $collection->where('price_type', $currency)->where("paybyus" , "0")->sum("revenue_partner") +
                        $collection->where('price_type', $currency)->where("paybyus" , "0")->sum("admin_partner");
                //dd($sum);
...
" >
    ...
    background: #fff;
">
            <h5 class="modal-title" id="exampleModalLabel" style="color:#E91E63;font-weight: bold;"><b>Today ENTRIES </b>
            <span class="caldn"><b>{{$tod1}}</b><small>{{$tod2}}</small><i class="far fa-calendar"></i></span>
            </h5>
                          ...
                          (
                            @foreach($row->users as $key=>$suser)
                                @if($key != 0)
                                    +
                                @endif
                                <small style="color:{{@$suser->m_user->background}}" title="{{@$suser->m_user->user_name}}">{{$suser->total}}</small>
    ...
    margin-top: 15px;
    margin-bottom: 0;
"><b style="color:#E91E63">Total:</b> <small style="
    font-size: 16px;
">{{$simsw}}</small></li>
                  </ul>
              </div>
                    ...
                    <div class="d-flex flex-row justify-content-between">
                        ...
                        <div>
                            ...
                            @if($aush->type == 1)
                                ...
                                @if($cons != 0)
                                <form action="{{ Route('panel.movement.view') }}" class="caldnf2" method="Get" role="form text-left">
                                ...
                                @if(auth()->user()->type == 1 && @$request["d_user"] == null && ( @$request["from_date"] == null || @$request["to_date"] == null))
                                    <ul class="munths wn showonmobile">
                                        ...
                                        ?>
                                        <li><a href="{{$url2.'&st=p'}}" class='{{@$request["st"] == "p" ? "selected" : "" }}'>Progress</a></li>
                                        <li><a href="{{$url2.'&st=c'}}" class='{{@$request["st"] == "c" ? "selected" : "" }}'>Completed</a></li>
                                        <li><a href="{{$url2.'&st=f'}}" class='{{@$request["st"] == "f" ? "selected" : "" }}'>Full</a></li>
                                    </ul>
                                @endif
                            @endif
                            <ul class="munths">
                            ...
                            <a  id="export_pdf" class="btn bg-gradient-dark btn-sm mb-2 export_pdf" type="button">Export PDF</a>
                            ...
                            <div class="col-lg-2 col-md-4">
                                <div class="form-group">
                                    ...
                                    <div class="@error('m_type') border border-danger rounded-3 @enderror">
                                        ...
                                        <option value="">Choose</option>
                                        <option value="Transfers" {{ isset($request["m_type"]) && $request["m_type"] == "Transfers" ? 'selected' : '' }}>Transfers</option>
                                        ...
                                        <option value="hotels" {{ isset($request["m_type"]) && $request["m_type"] == "hotels" ? 'selected' : '' }}>Hotels</option>
                                        <option value="Flights" {{ isset($request["m_type"]) && $request["m_type"] == "Flights" ? 'selected' : '' }}>Flights</option>
                                        <option value="T & T" {{ isset($request["m_type"]) && $request["m_type"] == "T & T" ? 'selected' : '' }}>T & T</option>
                ...
                <div id="exp_pdf" class="card-body px-3 pt-0 pb-2 cls">
                    <div class="card-header pb-0">
                        <div class="d-flex flex-row justify-content-between mb-2">
                            <div>
                                ...
                                ?>
                                <h5 class="mb-1">All entries ({{ $NowCount }}{{ isset($ispartner) && $ispartner->type == 5 ? " & ".@$NowCount2 : ''}})
                                    @if(isset($ispartner))
                                        @if($ispartner->type == 5)
                                        , Ahlan Antalya & {{ $ispartner->full_name }}{{ ($ispartner->s_usd != 0 || $ispartner->s_p != 0 || $ispartner->s_e != 0 || $ispartner->s_tl != 0) == true ? ": " : "" }}
                                        <!--    <span>-->
                                        ...
                                        <!--    </span>-->
                                        <!--    @if($aush->type == 1 && $ispartner->blance_usd != 0 || $ispartner->blance_tl != 0 || $ispartner->blance_e != 0 || $ispartner->blance_p != 0)-->
                                        ...
                                        @if(($ispartner->type == 2 || $ispartner->type == 4 || $ispartner->type == 3 || $ispartner->type == 5) && ($ispartner->id != 25 && $ispartner->id != 27 && $ispartner->id != 34))
                                            ...
                                            @if($aush->type == 1)
                                                @if($aush->type == 1 && $ispartner->s_usd != 0 || $ispartner->s_p != 0 || $ispartner->s_e != 0 || $ispartner->s_tl != 0)
                                                <div class="checkbox sus" style="margin-top: 0;width: auto;display: inline-block;float: none;margin-bottom: -11px;margin-right:10px;">
                                                    <input name="paids" type="checkbox" placeholder="paids" id="paids" value="1">
                                                    <label for="paids"> <span>,</span> {{ __('Paid Done') }}</label>
                                                </div>
                    ...
                    @foreach($data as $key=>$ye)
                        @foreach($ye as $key_yes => $yes)
                            ...
                            @if((isset($request["d_user"]) && $request["d_user"] != null) && (isset($user_m) && $user_m->type != 1) && auth()->user()->type != 1)
                            @else
                                @if(isset($ispartner) &&  $ispartner->type == 5)
  ...
  @section('panel_js')
    <script>
        $( document ).ready(function() {
            ...
            if(lens >= 1){
                $('html, body').animate({scrollTop: $(".table").find(".sredcol").offset().top - 70 });
